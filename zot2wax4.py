...
def create_and_copy():
    ...
    # Extract the name of the enveloping directory from the CSV file name
    enveloping_directory = os.path.splitext(os.path.basename(csv_file_path))[0]
    ...
    # Create the enveloping directory
    os.makedirs(enveloping_directory, exist_ok=True)
    print(f"Created enveloping directory '{enveloping_directory}'")
    ...
    with open(csv_file_path, 'r', encoding='utf-8-sig') as csvfile:
        ...
        for row in reader:
            if len(row) > max(key_column_index, file_attachments_column_index):
                new_directory = os.path.join(enveloping_directory, row[key_column_index].strip())
